...
function ajax(url, data, success, error, options) {
    ...
    xhr.onload = function () {
        ...
        if (responseType === "buffer") {
            try {
                filter(xhr.response, status, msg, responseHeaders);
            } catch (e) {
                error(500, "返回的数据格式非法");
            }
        } else if (responseType === "json" && xhr.responseText) {
            try {
                filter(JSON.parse(xhr.responseText), status, msg, responseHeaders);
            } catch (e) {
                error(500, "返回的数据格式非法");
            }
            ...
            try {
                filter(xhr.responseText, status, msg, responseHeaders);
            } catch (e) {
                error(500, "返回的数据格式非法");
            }
        }
